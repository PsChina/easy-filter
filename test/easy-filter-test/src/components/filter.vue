<template>
  <div>
    ...
    <div v-if="useOption">
      <div>ignore:</div>
      <div v-for="key in ['name','sex','age','id']" :key="key">
        <label :for="key">
          <span v-text="key"></span>
          <input type="checkbox" :value="key" @change="select" />
        </label>
      </div>
    </div>
    ...
    <table>
      ...
      <tr v-for="value in filter(personArray, useOption ? options : match)" :key="value.id">
