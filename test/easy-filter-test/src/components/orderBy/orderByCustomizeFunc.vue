...
@Component
export default class OrderByCustomizeTest extends Mixins(TestOrderby) {
  public readonly testCases: Array<CheckItem> = [
    {
      title: '测试自定义函数',
      className: 'customize-func',
      data: [{name:'Bar', age: 37}, {name:'Foo', age: 20}],
      func: (item1: any, item2: any, key: string, reverse: boolean) => item1[key]<item2[key] ? (reverse ? -1 : 1) : (reverse ? 1 : -1),
      doNotCheck: true
    }
  ];
  public test(): void {
    const tds = (document as any).querySelector('.customize-func').querySelectorAll('td') || [];
