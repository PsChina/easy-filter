...
function childExists(obj, match) {
  for (let key in obj) {
    ...
    if (type === "string" || type === "number") {
      if (
        new RegExp(match).test(value) ||
        value.toString().indexOf(match) !== -1
      ) {
        return true;
      }
    } else {
...
function limitTo(
  ...
  { startWithIndex = 0, startWith, ignore, cutOut = false } = {}
) {
  const type = typeof input;
  switch (type) {
    case "string": {
    ...
    default: {
      if (input instanceof Array) {
        ...
        if (
          type === "number" ||
          (input !== undefined && input !== null && "toString" in input)
        ) {
